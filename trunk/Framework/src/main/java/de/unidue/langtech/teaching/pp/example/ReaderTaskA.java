    ...
    extends JCasCollectionReader_ImplBase
{
        ...
        throws IOException, CollectionException
    {
        ...
        originalsentiment.setSentiment(parts[4]);
        originalsentiment.addToIndexes();
        
        SentimentIndex index = new SentimentIndex(jcas);
        index.setBeginIndex(Integer.parseInt(parts[2]));
        index.setEndIndex(Integer.parseInt(parts[3]));
        index.addToIndexes();

        // add actual text of the document, e.g. the tweet
        jcas.setDocumentText(parts[5]);
