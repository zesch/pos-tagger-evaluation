...
/**
 * Writes a specific Conll File (14-15 TAB separated) annotation from the CAS object. Format:
 * 
 * <pre>
    ...
    private void convert(JCas aJCas, PrintWriter aOut)
    {
        for (Sentence sentence : select(aJCas, Sentence.class)) {
            ...
            for (Row row : ctokens.values()) {
                ...
                String ppos = UNUSED;
                String pfeats = UNUSED;
                String phead = UNUSED;
                String pdeprel = UNUSED;
                String fillpred = UNUSED;
                String pred = UNUSED;
                String apreds = UNUSED;

                aOut.printf("%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\n", row.id,
                        row.token.getCoveredText(), lemma, plemma, pos, ppos, feats, pfeats, head, phead, 
                        deprel, pdeprel, fillpred, pred, apreds);
            }
