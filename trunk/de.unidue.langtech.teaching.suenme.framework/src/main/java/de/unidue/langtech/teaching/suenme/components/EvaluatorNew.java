...
public class EvaluatorNew {
	...
	public static void evaluateAll(AnalysisEngineDescription[] tagger, CollectionReaderDescription corpus) throws IOException {
    	...
    	saveTable(posTags, columnNamesPos, new File(dkproHome + "\\" + corpus.getImplementationName() + "-result-formatted-fine.txt"));
    	saveTable(cPosTags, columnNamesCPos, new File(dkproHome + "\\" + corpus.getImplementationName() + "-result-formatted-coarse.txt"));    	
	...
	public static void saveTable(String[][] tableArray, String[] columnNames, File file) throws IOException {
		
		for (int i = 0; i<columnNames.length; i++) {
			FileUtils.writeStringToFile(file, columnNames[i] + "\t", true);
		}
		
	   for(int i = 0; i<tableArray.length; i++) {
		   
	      for(int j = 0; j<tableArray[0].length; j++) {
	    	  FileUtils.writeStringToFile(file, tableArray[i][j] + "\t", true);
